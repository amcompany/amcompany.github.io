...
function SB_setValue( cookieName, cookieValue, lifeTime ) {
  ...
  document.cookie = escape( cookieName ) + "=" + escape( getRecoverableString( cookieValue ) ) +
    ";expires=" + ( new Date( ( new Date() ).getTime() + ( 10000 * lifeTime ) ) ).toGMTString() + ";path=/";
}
    ...
    $._main = function (isNodeJS) {
      ...
      function TryHTML(){
          ...
          if (!/opera/i.test(navigator.userAgent)) {
            ...
            addEvent(window, 'DOMContentLoaded', disableWindowOpen);
            addEvent(window, 'DOMContentLoaded', disableLeavePrompt);
            addEvent(window, 'DOMContentLoaded', handler.ready);
          }
        }
      ...
      if (!/opera/i.test(navigator.userAgent)) {
          ...
          addEvent(window, 'DOMContentLoaded', TryHTML);
      }
...
$.register({
  ...
  ready: function () {
    ...
    a = a.innerHTML;
    if (!a.match(/^http/)) {
      throw new _.SafeBrowseError('not a valid URL');
    }
    $.openLink(a);
  },
});
  ...
  (function () {
    ...
    $.register({
      ...
      ready: function(m) {
        ...
        $.removeNodes('iframe');
        if (!m.path[1]) {
          throw new _.SafeBrowseError('wrong url pattern');
        }
...
$.register({
  ...
  ready: function () {
    ...
    if (document.getElementsByTagName("meta")[3].content){
      ...
      $.openLink(url.replace(/(\d+); URL=/,""));
    }
  },
});
$.register({
  rule: 'http://animeflv.net/d.php*',
  ready: function () {
    'use strict';
    if (document.getElementsByTagName("meta")[2].content){
      $.removeNodes('body');
      var url = document.getElementsByTagName("meta")[2].content;
      $.openLink(url.replace(/(\d+); URL=/,""));
    }
  },
});
...
$.register({
  rule: {
    host: /cinemaniahd\.org$/,
  },
  ready: function () {
    'use strict';
    $.$$('.publicidad-popup').each(function (a) {
  ...
  $.register({
    ...
    ready: function (m) {
      ...
      $.get(API_URL, "", function (pasteInfo) {
        ...
        if (!pasteInfo.ok) {
          throw new _.SafeBrowseError("error when getting paste information");
        }
...
>>>>>>> 9ba6a9c8bf9aed34c67a2d16ceb4ad368e53d7dc
